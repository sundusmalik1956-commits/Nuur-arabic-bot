...
"""
lesson_engine.py
المحرك المشترك الذي تستدعيه كل ملفات <level>/lessonN.py عبر واجهة موحّدة send_lesson().
مسؤول عن:
  - إرسال كل مهارة (تمهيد/مفردات/قواعد/قراءة/استماع/محادثة/كتابة) في وقتها عبر JobQueue
    (بدون أي مهمة معلّقة تنتظر asyncio.sleep طوال الدرس).
  - بناء أزرار الاختيار من متعدد وتسجيل كل إجابة في قاعدة البيانات (مفصولة حسب المستوى).
  - عدم اعتبار الدرس مكتملًا إلا بعد تحقّق شروط إتمام كل مهاراته فعليًا.
  - إعلان الإنجاز في قروب المناقشة بعد التحقق الفعلي فقط.

هيكل بيانات الدرس المتوقع من كل <level>/lessonN.py (dict LESSON) موثّق في lesson_schema.py

المستويات: كل مستوى (a0/a1/a2/b1/b2) مجلد بايثون منفصل يحتوي lesson1.py...lessonN.py.
كل دالة هنا تأخذ user_id + level معًا، لأن رقم الدرس وحده لا يميّز الدرس (lesson1
في a1 مختلف تمامًا عن lesson1 في b1) — الفصل الكامل بينهما في قاعدة البيانات وفي تحميل الملف.
"""
...
def _load_lesson_module(level: str, lesson_number: int):
    """يحمّل <level>/lessonN.py ديناميكيًا، مثل a1.lesson1 أو b2.lesson7."""
    module_name = f"{level.lower()}.lesson{lesson_number}"
    try:
...
async def send_lesson(bot, user_id: int, language: str, level: str, lesson_number: int,
                       context: ContextTypes.DEFAULT_TYPE):
    """تبدأ درسًا لطالب (تُستدعى من الجدولة اليومية أو /lesson للاختبار)."""
    lesson = _load_lesson_module(level, lesson_number)
    if lesson is None:
        logger.warning(f"لا يوجد محتوى بعد للدرس {level}/lesson{lesson_number} (المستخدم {user_id}).")
        return

    db.reset_lesson_progress(user_id, level, lesson_number)
    db.mark_lesson_started_today(user_id)

    await _send_step(context, user_id, level, lesson_number, step_index=0)


def _build_choice_keyboard(level: str, lesson_number: int, skill: str, question_key: str, options: list) -> InlineKeyboardMarkup:
    buttons = []
    for i, opt in enumerate(options):
        callback_data = f"ans|{level}|{lesson_number}|{skill}|{question_key}|{i}"
        buttons.append([InlineKeyboardButton(opt, callback_data=callback_data)])
    return InlineKeyboardMarkup(buttons)


async def _send_step(context: ContextTypes.DEFAULT_TYPE, user_id: int, level: str, lesson_number: int, step_index: int):
    try:
        await _send_step_inner(context, user_id, level, lesson_number, step_index)
    except Exception:
        logger.exception(f"خطأ أثناء إرسال خطوة الدرس {level}/lesson{lesson_number} (step {step_index}) للمستخدم {user_id}")
...
async def _send_step_inner(context: ContextTypes.DEFAULT_TYPE, user_id: int, level: str, lesson_number: int, step_index: int):
    user = db.get_user(user_id)
    ...
    if step_index >= len(steps):
        await _finalize_lesson(context, user_id, level, lesson_number, lesson, lang)
        return
    ...
    db.start_skill(user_id, level, lesson_number, skill)

    if skill == "intro":
        await _send_intro(context, user_id, lang, step, title)
        db.complete_skill(user_id, level, lesson_number, skill)

    elif skill == "vocab":
        await _send_vocab(context, user_id, lang, level, lesson_number, step, title)

    elif skill == "grammar":
        await _send_grammar(context, user_id, lang, level, lesson_number, step, title)

    elif skill == "reading":
        await _send_reading(context, user_id, lang, level, lesson_number, step, title)

    elif skill == "listening":
        await _send_listening(context, user_id, lang, level, lesson_number, step, title)
    ...
    context.job_queue.run_once(
        _step_job,
        when=delay_seconds,
        data={"user_id": user_id, "level": level, "lesson_number": lesson_number, "step_index": next_index},
        name=f"lesson_step_{user_id}_{level}_{lesson_number}_{next_index}",
    )


async def _step_job(context: ContextTypes.DEFAULT_TYPE):
    data = context.job.data
    await _send_step(context, data["user_id"], data["level"], data["lesson_number"], data["step_index"])
...
async def _send_intro(context, user_id, lang, step, title):
...
async def _send_vocab(context, user_id, lang, level, lesson_number, step, title):
    lines = []
    ...
    if exercises:
        await _send_exercises(context, user_id, lang, level, lesson_number, "vocab", exercises)
    else:
        db.complete_skill(user_id, level, lesson_number, "vocab")


async def _send_grammar(context, user_id, lang, level, lesson_number, step, title):
    ...
    exercises = step.get("exercises", [])
    if exercises:
        await _send_exercises(context, user_id, lang, level, lesson_number, "grammar", exercises)
    else:
        db.complete_skill(user_id, level, lesson_number, "grammar")


async def _send_reading(context, user_id, lang, level, lesson_number, step, title):
    ...
    exercises = step.get("exercises", [])
    if exercises:
        await _send_exercises(context, user_id, lang, level, lesson_number, "reading", exercises)
    else:
        db.complete_skill(user_id, level, lesson_number, "reading")


async def _send_listening(context, user_id, lang, level, lesson_number, step, title):
    image = step.get("image")
    if image:
    ...
    exercises = step.get("exercises", [])
    if exercises:
        await _send_exercises(context, user_id, lang, level, lesson_number, "listening", exercises)
    else:
        db.complete_skill(user_id, level, lesson_number, "listening")
...
async def _send_exercises(context, user_id, lang, level, lesson_number, skill, exercises):
    for i, exercise in enumerate(exercises):
        ...
        keyboard = _build_choice_keyboard(level, lesson_number, skill, question_key, options)
...
async def handle_answer_callback(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _, level, lesson_number_str, skill, question_key, choice_index_str = query.data.split("|")
    ...
    db.record_answer(user_id, level, lesson_number, skill, question_key, selected_text, is_correct)
    ...
    if is_correct:
        ...
        if db.all_questions_answered_correctly(user_id, level, lesson_number, skill, all_keys):
            db.complete_skill(user_id, level, lesson_number, skill)
    else:
        keyboard = _build_choice_keyboard(level, lesson_number, skill, question_key, options)
        try:
...
def get_active_ai_skill(user_id: int):
    ...
    level = user["level"]
    lesson_number = user["current_lesson"]
    completed = db.get_completed_skills(user_id, level, lesson_number)
    for skill in ("speaking", "writing"):
        with db.get_conn() as conn:
            row = conn.execute(
                """SELECT status FROM lesson_progress
                   WHERE user_id=? AND level=? AND lesson_number=? AND skill=?""",
                (user_id, level, lesson_number, skill),
            ).fetchone()
...
def _lesson_fully_completed(user_id: int, level: str, lesson_number: int, lesson: dict) -> bool:
    required_skills = {step["skill"] for step in lesson["steps"]}
    completed = db.get_completed_skills(user_id, level, lesson_number)
    return required_skills.issubset(completed)


async def _finalize_lesson(context: ContextTypes.DEFAULT_TYPE, user_id: int, level: str,
                            lesson_number: int, lesson: dict, lang: str):
    if not _lesson_fully_completed(user_id, level, lesson_number, lesson):
        ...
        return
    await _complete_lesson_now(context, user_id, level, lesson_number, lesson, lang)
...
async def check_and_complete_if_ready(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if lesson is None:
        return
    lang = user.get("language", "ar")
    if _lesson_fully_completed(user_id, level, lesson_number, lesson):
        if not db.is_completion_logged(user_id, level, lesson_number):
            await _complete_lesson_now(context, user_id, level, lesson_number, lesson, lang)


async def _complete_lesson_now(context: ContextTypes.DEFAULT_TYPE, user_id: int, level: str,
                                lesson_number: int, lesson: dict, lang: str):
    ...
    db.log_completion(user_id, level, lesson_number, title)
    db.advance_to_next_lesson(user_id)

    await _announce_achievement(context, user_id, level, lesson_number, title)
...
async def _announce_achievement(context: ContextTypes.DEFAULT_TYPE, user_id: int, level: str,
    ...
    text = (
        ...
        f"📚 المستوى {level} — أتم الدرس {lesson_number}:\n{lesson_title}\n\n"
