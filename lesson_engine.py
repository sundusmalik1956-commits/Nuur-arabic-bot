...
def _load_lesson_module(level_number: int, lesson_number: int):
    ...
    try:
        ...
        logger.exception(f"خطأ أثناء تحميل {module_path}")
        return None
...
async def _send_step(context: ContextTypes.DEFAULT_TYPE, user_id: int, level_number: int, lesson_number: int, step_index: int):
    try:
        await _send_step_inner(context, user_id, level_number, lesson_number, step_index)
    except Exception:
...
async def _send_step_inner(context: ContextTypes.DEFAULT_TYPE, user_id: int, level_number: int, lesson_number: int, step_index: int):
    user = db.get_user(user_id)
    ...
    if step_index >= len(steps):
        await _finalize_lesson(context, user_id, level_number, lesson_number, lesson, lang)
        return
    ...
    if skill == "intro":
        ...
        await _send_step(context, user_id, level_number, lesson_number, step_index + 1)

    elif skill == "vocab":
        await _send_vocab(context, user_id, lang, level_number, lesson_number, step, title, step_index)

    elif skill == "grammar":
        await _send_grammar(context, user_id, lang, level_number, lesson_number, step, title, step_index)

    elif skill == "reading":
        await _send_reading(context, user_id, lang, level_number, lesson_number, step, title, step_index)

    elif skill == "listening":
        await _send_listening(context, user_id, lang, level_number, lesson_number, step, title, step_index)
...
async def _send_vocab(context, user_id, lang, level_number, lesson_number, step, title, step_index):
    lines = []
    ...
    if exercises:
        await _send_exercises(context, user_id, lang, level_number, lesson_number, "vocab", exercises, step_index)
    else:
        db.complete_skill(user_id, lesson_number, "vocab")
        await _send_step(context, user_id, level_number, lesson_number, step_index + 1)


async def _send_grammar(context, user_id, lang, level_number, lesson_number, step, title, step_index):
    ...
    exercises = step.get("exercises", [])
    if exercises:
        await _send_exercises(context, user_id, lang, level_number, lesson_number, "grammar", exercises, step_index)
    else:
        db.complete_skill(user_id, lesson_number, "grammar")
        await _send_step(context, user_id, level_number, lesson_number, step_index + 1)


async def _send_reading(context, user_id, lang, level_number, lesson_number, step, title, step_index):
    ...
    exercises = step.get("exercises", [])
    if exercises:
        await _send_exercises(context, user_id, lang, level_number, lesson_number, "reading", exercises, step_index)
    else:
        db.complete_skill(user_id, lesson_number, "reading")
        await _send_step(context, user_id, level_number, lesson_number, step_index + 1)


async def _send_listening(context, user_id, lang, level_number, lesson_number, step, title, step_index):
    image = step.get("image")
    if image:
    ...
    exercises = step.get("exercises", [])
    if exercises:
        await _send_exercises(context, user_id, lang, level_number, lesson_number, "listening", exercises, step_index)
    else:
...
async def _send_exercises(context, user_id, lang, level_number, lesson_number, skill, exercises, step_index):
...
async def _finalize_lesson(context: ContextTypes.DEFAULT_TYPE, user_id: int, level_number: int, lesson_number: int,
